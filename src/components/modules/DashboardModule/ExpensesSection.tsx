...
const ExpensesSection = () => {
  const { paramFilter, refetch, setRefetch } = useContext(DashboardContext)
  const page = 1
  const [expensesData, setExpensesData] = useState<APIProps>()
  useEffect(() => {
    if (refetch) {
      setRefetch(false);
      const urlTarget = `https://utbmu5o3smxuba2iverkgqqj440temhn.lambda-url.ap-southeast-1.on.aws/expenses?page=${page}` + (paramFilter !== "" ? `&category_id=${paramFilter}` : "");
